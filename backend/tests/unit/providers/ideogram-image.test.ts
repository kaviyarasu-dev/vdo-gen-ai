import { describe, it, expect, vi, beforeEach } from 'vitest';
import { IdeogramImageAdapter } from '../../../src/providers/image-generation/ideogram-image.adapter.js';
import { ProviderError, ProviderRateLimitError } from '../../../src/providers/provider.errors.js';

// Mock global fetch
const mockFetch = vi.fn();
vi.stubGlobal('fetch', mockFetch);

describe('IdeogramImageAdapter', () => {
  let adapter: IdeogramImageAdapter;

  beforeEach(() => {
    vi.clearAllMocks();
    adapter = new IdeogramImageAdapter('ideo-test-key');
  });

  describe('metadata', () => {
    it('should have correct slug, displayName, and category', () => {
      expect(adapter.slug).toBe('ideogram');
      expect(adapter.displayName).toBe('Ideogram');
      expect(adapter.category).toBe('image-generation');
    });

    it('should be webhook-based', () => {
      expect(adapter.isWebhookBased()).toBe(true);
    });

    it('should return webhook signature header', () => {
      expect(adapter.getWebhookSignatureHeader()).toBe('authorization');
    });
  });

  describe('listModels', () => {
    it('should return available models', async () => {
      const models = await adapter.listModels();
      expect(models.length).toBeGreaterThanOrEqual(2);
      expect(models[0].id).toBe('ideogram-v2');
      expect(models[0].capabilities).toContain('text-to-image');
    });
  });

  describe('verifyWebhookSignature', () => {
    it('should verify valid bearer token signature', () => {
      const payload = Buffer.from('{"event":"completed"}');
      const secret = 'my-webhook-secret';
      const signature = `Bearer ${secret}`;

      expect(adapter.verifyWebhookSignature(payload, signature, secret)).toBe(true);
    });

    it('should reject invalid bearer token signature', () => {
      const payload = Buffer.from('{"event":"completed"}');
      expect(adapter.verifyWebhookSignature(payload, 'Bearer wrong-secret', 'my-webhook-secret')).toBe(false);
    });

    it('should reject mismatched length signatures', () => {
      const payload = Buffer.from('{"event":"completed"}');
      expect(adapter.verifyWebhookSignature(payload, 'short', 'my-webhook-secret')).toBe(false);
    });
  });

  describe('generate', () => {
    it('should return completed result when data is immediately available', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          request_id: 'req-ideo-123',
          data: [
            {
              url: 'https://ideogram.ai/result/image1.png',
              resolution: '1024x1024',
              is_image_safe: true,
              prompt: 'A beautiful sunset',
            },
          ],
        }),
      });

      const result = await adapter.generate({
        prompt: 'A beautiful sunset over mountains',
        model: 'ideogram-v2',
        width: 1024,
        height: 1024,
      });

      expect(result.status).toBe('completed');
      expect(result.externalId).toBe('req-ideo-123');
      expect(result.images).toHaveLength(1);
      expect(result.images![0].url).toBe('https://ideogram.ai/result/image1.png');
    });

    it('should return pending when data is not immediately available', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          request_id: 'req-ideo-456',
          data: [],
        }),
      });

      const result = await adapter.generate({
        prompt: 'A complex scene',
        model: 'ideogram-v2',
        width: 1024,
        height: 1024,
      });

      expect(result.status).toBe('pending');
      expect(result.externalId).toBe('req-ideo-456');
      expect(result.estimatedWaitMs).toBe(30_000);
    });

    it('should include negative prompt and seed when provided', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          request_id: 'req-789',
          data: [{ url: 'https://ideogram.ai/img.png', resolution: '1024x1024', is_image_safe: true, prompt: 'test' }],
        }),
      });

      await adapter.generate({
        prompt: 'A cat',
        negativePrompt: 'blurry',
        model: 'ideogram-v2',
        width: 1024,
        height: 1024,
        seed: 42,
      });

      const callBody = JSON.parse(mockFetch.mock.calls[0][1].body);
      expect(callBody.image_request.negative_prompt).toBe('blurry');
      expect(callBody.image_request.seed).toBe(42);
    });

    it('should handle API errors', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 400,
        text: async () => 'Invalid prompt',
      });

      await expect(
        adapter.generate({
          prompt: 'test',
          model: 'ideogram-v2',
          width: 1024,
          height: 1024,
        }),
      ).rejects.toThrow(ProviderError);
    });
  });

  describe('checkStatus', () => {
    it('should map COMPLETED status correctly', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          request_id: 'req-123',
          status: 'COMPLETED',
        }),
      });

      const status = await adapter.checkStatus('req-123');
      expect(status.status).toBe('completed');
    });

    it('should map PROCESSING status correctly', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          request_id: 'req-123',
          status: 'PROCESSING',
        }),
      });

      const status = await adapter.checkStatus('req-123');
      expect(status.status).toBe('processing');
    });

    it('should map PENDING status to queued', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          request_id: 'req-123',
          status: 'PENDING',
        }),
      });

      const status = await adapter.checkStatus('req-123');
      expect(status.status).toBe('queued');
    });

    it('should map FAILED status correctly', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          request_id: 'req-123',
          status: 'FAILED',
          error: 'Content policy violation',
        }),
      });

      const status = await adapter.checkStatus('req-123');
      expect(status.status).toBe('failed');
      expect(status.error).toBe('Content policy violation');
    });

    it('should handle API errors during status check', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 500,
        text: async () => 'Internal error',
      });

      await expect(adapter.checkStatus('req-123')).rejects.toThrow(ProviderError);
    });
  });

  describe('getResult', () => {
    it('should return completed result with images', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          request_id: 'req-123',
          status: 'COMPLETED',
          data: [
            { url: 'https://ideogram.ai/result.png', resolution: '1024x1024', is_image_safe: true, prompt: 'test' },
          ],
        }),
      });

      const result = await adapter.getResult('req-123');
      expect(result.status).toBe('completed');
      expect(result.externalId).toBe('req-123');
      expect(result.images).toHaveLength(1);
      expect(result.images![0].url).toBe('https://ideogram.ai/result.png');
    });

    it('should return failed for failed tasks', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          request_id: 'req-123',
          status: 'FAILED',
          error: 'Generation failed due to content policy',
        }),
      });

      const result = await adapter.getResult('req-123');
      expect(result.status).toBe('failed');
      expect(result.error).toBe('Generation failed due to content policy');
    });

    it('should return failed when no images in result', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          request_id: 'req-123',
          status: 'COMPLETED',
          data: [],
        }),
      });

      const result = await adapter.getResult('req-123');
      expect(result.status).toBe('failed');
      expect(result.error).toBe('No images in result');
    });
  });

  describe('error handling', () => {
    it('should translate rate limit to ProviderRateLimitError', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 429,
        text: async () => 'Rate limit exceeded',
      });

      await expect(
        adapter.generate({
          prompt: 'test',
          model: 'ideogram-v2',
          width: 1024,
          height: 1024,
        }),
      ).rejects.toThrow(ProviderRateLimitError);
    });

    it('should translate server errors to retryable ProviderError', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 500,
        text: async () => 'Internal server error',
      });

      try {
        await adapter.generate({
          prompt: 'test',
          model: 'ideogram-v2',
          width: 1024,
          height: 1024,
        });
        expect.fail('Should have thrown');
      } catch (error) {
        expect(error).toBeInstanceOf(ProviderError);
        expect((error as ProviderError).retryable).toBe(true);
      }
    });

    it('should translate 401 errors with correct status code', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 401,
        text: async () => 'Unauthorized',
      });

      try {
        await adapter.generate({
          prompt: 'test',
          model: 'ideogram-v2',
          width: 1024,
          height: 1024,
        });
        expect.fail('Should have thrown');
      } catch (error) {
        expect(error).toBeInstanceOf(ProviderError);
        expect((error as ProviderError).statusCode).toBe(401);
      }
    });
  });
});
