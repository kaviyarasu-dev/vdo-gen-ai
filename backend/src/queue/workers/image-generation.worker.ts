...
): Worker<ImageGenerationJobData, JobResult> {
  const executionService = new WorkflowExecutionService();
  const nodeRegistry = getNodeRegistry();

  const worker = new Worker<ImageGenerationJobData, JobResult>(
    ...
    async (job: Job<ImageGenerationJobData>) => {
      ...
      try {
        ...
        if ((job.attemptsMade ?? 0) + 1 >= (job.opts?.attempts ?? RETRY_CONFIG.imageGeneration.attempts)) {
    ...
    {
      connection,
      concurrency: WORKER_CONFIG[QUEUES.IMAGE_GENERATION].concurrency,
      autorun: true,
    },
  );

  worker.on('failed', (job, err) => {
    logger.error({ jobId: job?.id, err }, 'Image generation worker job failed');

    if (job) {
      const maxAttempts = job.opts?.attempts ?? RETRY_CONFIG.imageGeneration.attempts;
