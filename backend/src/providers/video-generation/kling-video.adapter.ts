...
export class KlingVideoAdapter implements IVideoGenerationProvider {
  readonly slug = SLUG;
  ...
  async validateCredentials(apiKey: string): Promise<boolean> {
    try {
      const response = await fetch(`${BASE_URL}/videos/tasks/test-nonexistent`, {
        method: 'GET',
    ...
    return [
      {
        id: 'kling-v1',
        name: 'Kling V1',
        description: 'High-quality video generation with cinematic style',
      ...
      },
      {
        id: 'kling-v1-5',
        name: 'Kling V1.5',
        description: 'Latest generation model with improved motion fidelity',
        capabilities: ['image-to-video'],
        maxResolution: { width: 1920, height: 1080 },
        maxDuration: 10,
      },
  ...
  async submit(input: VideoGenerationInput): Promise<VideoSubmissionResult> {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/videos/image-to-video`, {
        method: 'POST',
      ...
      return {
        ...
        status: task.task_status === 'processing' ? 'processing' : 'queued',
  ...
  async checkStatus(externalId: string): Promise<VideoGenerationStatus> {
    try {
      const response = await fetch(`${BASE_URL}/videos/tasks/${externalId}`, {
        method: 'GET',
        headers: this.buildHeaders(),
      });
    ...
    try {
      ...
      if (task.task_status === 'failed') {
        return {
          status: 'failed',
          externalId,
          error: task.task_status_msg ?? 'Video generation failed',
        };
      }

      if (task.task_status !== 'succeed') {
        throw new ProviderError(
          `Task ${externalId} is not completed (status: ${task.task_status})`,
          SLUG,
          { retryable: true },
        );
      }
  ...
  private async buildFetchError(response: Response): Promise<{ status: number; message: string }> {
    let message = `Kling API error (${response.status})`;
    try {
  ...
  private mapError(error: unknown): ProviderError {
    ...
    const message = errObj.error?.message ?? errObj.message ?? 'Unknown Kling error';
    ...
    logger.error({ err: error }, 'Kling API error');
...
function mapKlingStatus(status: string): VideoGenerationStatus['status'] {
  switch (status) {
    case 'submitted':
      return 'queued';
    case 'processing':
